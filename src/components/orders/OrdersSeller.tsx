import type { TypeMyOrdersSeller } from "../../types/orders.types";
import { motion } from "framer-motion";
import { AnimatePresence } from "framer-motion";
import { Calendar, CreditCard, Hash, ChevronUp, Package, User } from "lucide-react";
import { formatDate, formatDateTime } from "../../utils/formatDate";
import { formatPrice } from "../../utils/formatPrice";

export interface Props {
  orders: TypeMyOrdersSeller[];
  expandedOrders: Set<string>;
  toggleOrder: (orderId: string) => void;
}

export const OrdersSeller = ({ orders, expandedOrders, toggleOrder }: Props) => {
  return (
    <div className="space-y-3 sm:space-y-4">
    {orders.map((order: TypeMyOrdersSeller, index: number) => {
                          ...
                          <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
                            <div className="flex-1 min-w-0">
                              ...
                              <div className="flex items-center gap-1.5 mb-2">
                                <User className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-muted shrink-0" />
                                <p className="text-xs sm:text-sm text-muted truncate">{item.buyer_name}</p>
                              </div>
