...
export const CartPage = () => {
  ...
  const handleCreateOrder = async () => {
    ...
    const orderData = transformOrderToCreate(cartItems);
    const { success, message, data } = await OrdersService.createOrder(
      orderData
    );
    if (success && data) {
      toast.success(message);
      // Abrir el link de PayPal en una ventana nueva
      if (data.link) {
  ...
  const handleUpdateQuantity = async (
    ...
    const { success, message } = await CartService.updateCartItem(
      productId,
      newQuantity
    );
          ...
          <div className="max-w-md w-full bg-white border border-neutral-200 rounded-2xl p-8 sm:p-12 text-center shadow-lg">
            ...
            <div className="bg-neutral-50 border border-neutral-200 rounded-xl p-4 mb-6">
              <div className="flex items-center justify-center gap-3 text-muted text-sm">
                ...
                <span>Procesando orden de pago</span>
              </div>
            </div>
                ...
                <div className="lg:col-span-2 space-y-4">
                  ...
                  {cartItems.map((item) => {
                    return (
                      <div
                        key={item.product_id}
                        className={`bg-white border border-neutral-200 rounded-xl p-4 sm:p-6 transition-all ${"hover:shadow-md"}`}
                      >
